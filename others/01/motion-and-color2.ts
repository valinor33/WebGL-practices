...
function movementAndColorDemo() {
  ...
  const indigoGradientSquareColorBuffer = createStaticVertexBuffer(
    gl,
  ...
  if (
    ...
    !fireyTriangleColorBuffer ||
    !squareGeoBuffer ||
    !indigoGradientSquareColorBuffer ||
    !graySquareColorsBuffer ||
    !circleInterleavedBuffer
  ) {
    showError(
      `Failed to create vertex buffers (triangle pos=${!!triangleGeoBuffer},` +
  ...
  const movementAndColorProgram = createProgram(
    gl,
  ...
  if (vertexPositionAttributeLocation < 0 || vertexColorAttributeLocation < 0) {
    showError(
      `Failed to get attribute locations: (pos=${vertexPositionAttributeLocation},` +
